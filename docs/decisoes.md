...
- **O ano é filtro DE CADA TELA, e começa sempre no ano atual.** Decisão do chefe em 2026-08-04, que desmontou o ano de contexto por MÓDULO. Aquele morava na navbar e guardava a escolha no `localStorage`: abrir o mapa de 2025 mudava calado a lista de pedidos, e voltar semanas depois abria num ano antigo sem nada avisar. Agora vale `@components/filtro-ano.js`. Cada tela monta o seu, como primeiro item da barra de filtros, e nada sobrevive à sessão. A diferença entre os módulos deixou de ser um componente e virou o parâmetro `permitirOutroAno`: no orçamento o ano também decide **onde se cadastra**, e por isso ele oferece "+ Outro ano…"; na mapoteca o ano só **filtra o que já aconteceu**, e um ano vazio seria uma tela em branco. O dashboard da mapoteca tem UM filtro, no nível da página, que vale para as cinco abas e chega a elas como função. Ficam sem ano a lista de **clientes**, que é cadastro e não movimento, e o **estoque**, que é o saldo de hoje. O custo, deliberado: o pedido de dezembro concluído em janeiro só aparece trocando o ano no filtro da tela.
- **No dashboard da mapoteca existem DOIS recortes anuais, e cada aba diz na tela qual é o dela.** Resumo Anual e Mapa contam por data de **entrega** (`FILTRO_ENTREGUE_ANO`); Pedidos e Atendimento, por data do **pedido** (`FILTRO_ANO_PEDIDO`). O pedido de dezembro de 2025 entregue em janeiro de 2026 cai em anos diferentes nos dois, e os dois estão certos. Sem a linha de escopo na tela (`.dashboard__escopo`) os números pareceriam se contradizer. A aba Materiais é meio a meio e também avisa: o consumo é do ano, mas o **estoque é o saldo de hoje** e ignora o filtro, porque "estoque de 2025" não existe; a rota dele é a única do dashboard que não aceita `ano`. As janelas deslizantes ("últimos 6 meses") saíram junto: em 2025 elas continuariam terminando hoje.
