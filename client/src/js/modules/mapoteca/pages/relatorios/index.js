...
import { getRelatorio, downloadRelatorioCsv } from '@modules/mapoteca/services/mapoteca-service.js';
...
function linkPedido(row) {
  ...
  return el('a', {
    href: `#/mapoteca/pedidos/${row.pedido_id}`,
...
const RELATORIOS = {
  ...
  impressao_detalhada: {
    label: 'Impressão detalhada',
    descricao: 'Um item de pedido por linha, no formato enxuto da planilha de impressão (15 colunas, sem nome do produto/mês). Filtra por ano do pedido. O CSV exportado contém apenas as 15 colunas; o localizador abaixo é só para navegação.',
    emptyMessage: 'Nenhum item de pedido no ano selecionado',
    columns: [
      { key: 'omds', label: 'OMDS' },
      { key: 'demandante', label: 'Demandante' },
  ...
  pedidos_resumo: {
    label: 'Resumo de pedidos (envio + consolidado)',
    descricao: 'Um pedido por linha (todos os clientes) com dados de envio e o consolidado de produtos entregues por tipo e escala. Filtra por ano do pedido.',
    emptyMessage: 'Nenhum pedido no ano selecionado',
    columns: [
      { key: 'numero_pedido', label: 'Número do Pedido', sortable: true },
      { key: 'unidade', label: 'Unidade', sortable: true },
      { key: 'documento', label: 'Documento (DIEx)' },
      { key: 'status', label: 'Status' },
      colData('data_envio', 'Data de Envio', true),
      { key: 'informacoes_envio', label: 'Informações de Envio' },
      colNum('topo_25k', 'Topo 25k'),
      colNum('topo_50k', 'Topo 50k'),
      colNum('topo_100k', 'Topo 100k'),
      colNum('topo_250k', 'Topo 250k'),
      colNum('total_topo', 'Total Topo'),
      colNum('orto_25k', 'Orto 25k'),
      colNum('orto_50k', 'Orto 50k'),
      colNum('orto_100k', 'Orto 100k'),
      colNum('orto_250k', 'Orto 250k'),
      colNum('total_orto', 'Total Orto'),
      colNum('outros_produtos', 'Outros'),
      colNum('produtos_digitais', 'Digitais'),
      { key: 'total', label: 'Total Entregue', sortable: true, render: (row) => formatNumber(row.total ?? 0) },
      { key: 'localizador_pedido', label: 'Localizador', render: linkPedido },
    ],
  },
  pedidos_civ: {
    ...
    columns: [
      ...
      colNum('qtd_imagens', 'Nº de Imagens'),
      colData('data_envio', 'Data Envio/Retirada'),
      { key: 'situacao', label: 'Status' },
      { key: 'observacao', label: 'Observação' },
      { key: 'localizador_pedido', label: 'Localizador', render: linkPedido },
