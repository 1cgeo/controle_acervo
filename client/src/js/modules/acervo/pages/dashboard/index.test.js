...
describe('renderDashboard do acervo', () => {
  test('monta o titulo, a barra de exportacao e as cinco abas', async () => {
    ...
    // SEM 'Plano do Ano', que saiu em 2026-08-07: das quatro tabelas dela so a
    // folha planejada era assunto do acervo, e essa foi para a Visão Geral.
    expect(rotulosAbas(container)).toEqual([
      'Visão Geral', 'Distribuição', 'Atividade',
  ...
  test('abre na visao geral e ja carrega os totais, nao os graficos', async () => {
    const container = document.createElement('div');
  ...
  test('o cleanup para o auto-refresh e desmonta a aba ativa', async () => {
    ...
    // A Visão Geral é cartão e tabela, sem gráfico. A Distribuição desenha, e é
    // pelo gráfico dela que dá para ver se a aba ATIVA foi desmontada. Com lista
