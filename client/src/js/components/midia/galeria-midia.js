...
/**
 * A galeria de fotos e vídeos de um registro, e a tela cheia dela.
 *
 * UM COMPONENTE PARA TODA MÍDIA DO SAP. Ele nasceu dentro da tela de campo
 * (`pages/campo/campo-midia.js`) e saiu de lá em 2026-09-15, quando a
 * capacitação (a 2.6 ministrada e a 6.2 recebida) passou a aceitar foto e vídeo
 * também. A alternativa era uma segunda galeria, e a que ficasse de fora da
 * correção seguinte seria a que todo mundo vê -- foi este mesmo argumento que
 * fez a ficha e o formulário do campo dividirem UMA galeria.
 *
 * O QUE MUDA ENTRE OS DONOS É O `servico`, e não a tela: cinco funções com a
 * mesma forma (listar, enviar, atualizar, excluir e a URL do arquivo). Quem as
 * amarra ao endereço de cada módulo é o arquivo da página, e não este.
 *
 * LEITURA E ESCRITA CONTINUAM SEPARADAS por `podeEditar`, que é a régua da casa:
 * abrir é VER, e o que muda o registro mora onde se diz que vai editar.
 */
 ...
 *
 * ESPELHA `server/src/utils/midia.js` (`MAX_BASE64`, 58.720.256 caracteres):
 * base64 cresce o binário em um terço, então o teto do arquivo é três quartos
 * daquele número -- 42 MiB. O teto de lá existe porque ele TEM de caber no
 * `express.json` de 60mb; com o corpo maior que isso, o body parser responde 413
 * e o Joi nunca roda, e a mensagem que sobra não fala nem de foto nem do
 * registro. O campo e a capacitação leem o MESMO número, dos dois lados.
 */
 ...
 * @param {Function} opts.urlDoArquivo - (id) => Promise<string>, a URL de blob
 * @param {string} [opts.alt] - o texto alternativo da imagem
 */
export function abrirTelaCheia({
  itens, indice = 0, urlDoArquivo, alt = 'Foto',
}) {
  let atual = indice;
  let urlAtual = null;

  const midia = el('div', { className: 'midia-luz__midia' });
  const legenda = el('div', { className: 'midia-luz__legenda' });
  const contador = el('span', { className: 'midia-luz__contador' });
  ...
  const desenhar = () => {
    ...
    midia.appendChild(el('span', {
      className: 'midia-luz__carregando', textContent: 'Carregando...',
    }));
    ...
    urlDoArquivo(item.id).then((url) => {
      ...
      midia.appendChild(item.tipo === 'video'
        ...
        : el('img', { src: url, alt: item.descricao || alt }));
    }).catch((err) => {
      if (pedido !== atual) return;
      clearChildren(midia);
      midia.appendChild(el('span', {
        className: 'midia-luz__erro',
  ...
  const anterior = el('button', {
    className: 'midia-luz__nav midia-luz__nav--antes',
    type: 'button',
    'aria-label': 'Anterior',
  ...
  const proximo = el('button', {
    className: 'midia-luz__nav midia-luz__nav--depois',
    type: 'button',
    'aria-label': 'Próximo',
    onClick: (e) => { e.stopPropagation(); ir(1); },
  ...
  const fundo = el('div', {
    ...
    el('div', { className: 'midia-luz__barra' }, [
      contador,
      el('button', {
        className: 'midia-luz__fechar',
        type: 'button',
        'aria-label': 'Fechar',
...
/**
 * A galeria de um registro.
 *
 * @param {Object} opts
 * @param {number} opts.id - o dono da mídia (um campo, uma capacitação)
 * @param {Object} opts.servico - listar, enviar, atualizar, excluir e url
 * @param {boolean} [opts.podeEditar] - mostra enviar e remover
 * @param {Function} [opts.aoMudar] - houve escrita; quem chamou recarrega
 * @param {Object} [opts.textos] - alt da imagem e as duas frases de vazio
 * @returns {{element:HTMLElement, recarregar:Function, cleanup:Function}}
 */
export function criarGaleriaMidia({
  id, servico, podeEditar = false, aoMudar = null, textos = {},
}) {
  // O QUE A TELA DIZ QUANDO NÃO HÁ NADA, e como a imagem se descreve a quem não
  // a vê. Os padrões são os do campo, que foi de onde este componente saiu.
  const alt = textos.alt || 'Foto';
  const vazio = textos.vazio || 'Nenhuma foto ou vídeo neste registro.';
  const vazioEditavel = textos.vazioEditavel
  ...
  // `<div class="midia-luz">` cobrindo a tela seguinte.
  let luz = null;

  const abrirLuz = (indice) => {
    if (luz) luz.fechar();
    luz = abrirTelaCheia({ itens, indice, urlDoArquivo: servico.url, alt });
  };

  const grade = el('div', { className: 'midia-galeria' });
  const acoes = el('div', { className: 'midia-galeria__barra' });
  ...
  if (podeEditar) {
    ...
    const entrada = el('input', {
      ...
      onChange: async (e) => {
        ...
        // `MAX_BASE64` do servidor. É a mesma regra do teto de
        ...
        try {
          ...
          // nem `campo.imagem` nem `rpcmtec.capacitacao_imagem` têm chave por
          // conteúdo que a recuse.
   ...
   */
  const subir = (alvo, arquivo) => new Promise((resolve, reject) => {
  ...
  const cartao = (imagem, indice) => {
    const midia = el('div', {
      className: 'midia-galeria__midia midia-galeria__midia--clicavel',
      role: 'button',
      tabindex: '0',
    ...
    }, [el('span', { className: 'midia-galeria__carregando', textContent: 'carregando...' })]);

    servico.url(imagem.id).then((url) => {
      ...
      midia.appendChild(imagem.tipo === 'video'
        ...
        : el('img', { src: url, alt: imagem.descricao || alt, loading: 'lazy' }));
      if (imagem.tipo === 'video') {
        midia.appendChild(el('span', { className: 'midia-galeria__marca-video' }, ['vídeo']));
      }
    }).catch((err) => {
      if (disposed) return;
      clearChildren(midia);
      midia.appendChild(el('span', {
        className: 'midia-galeria__erro',
        textContent: err.message || 'Não foi possível carregar este arquivo.',
      }));
    });

    const rodape = el('div', { className: 'midia-galeria__rodape' });
    const item = el('figure', { className: 'midia-galeria__item' }, [midia, rodape]);

    /**
     * O RODAPÉ TEM DOIS MODOS, e a descrição se edita ALI, no cartão.
     *
     * NÃO É UM MODAL: a galeria já vive DENTRO de um ("Editar o campo", "Fotos
     * e vídeos" da capacitação), e um segundo por cima esconderia qual dos dois
     * está gravando -- é a mesma razão pela qual a ficha do campo FECHA antes de
     * abrir o formulário.
     *
     * SÓ A DESCRIÇÃO. A linha de mídia tem `descricao` e `data_imagem`, e a
     * data saiu da tela por decisão do chefe em 2026-08-20 (ver
     * `legendaDoItem`). Ela viaja no PUT mesmo assim, com o valor que já estava:
     * o servidor grava `data_imagem = dados.data_imagem || null`, então OMITIR
     * apagaria a data das dez imagens antigas que a têm.
     */
    const desenharRodape = (editando) => {
      clearChildren(rodape);
      if (!editando) {
        rodape.append(
          el('span', { className: 'midia-galeria__descricao', textContent: rotulo(imagem) }),
      ...
      const entrada = el('input', {
        type: 'text',
        className: 'form-field__input midia-galeria__entrada',
      ...
      // GRAVA NA HORA, e não no "Salvar" de quem hospeda: o formulário do campo
      // não inclui a galeria no salvamento dele, e o modal de mídia da
      // capacitação não tem "Salvar" nenhum. A escrita da galeria é dela.
      async function salvar() {
        const novo = entrada.value.trim();
        if (novo === (imagem.descricao || '')) { desenharRodape(false); return; }
        entrada.disabled = true;
        try {
          await servico.atualizar(imagem.id, {
            descricao: novo || null,
          ...
          //
          // `data_imagem` VAI COM O VALOR GRAVADO: o servidor a sobrescreve a
          // cada PUT, e omiti-la apagaria a data de quem a tem.
      ...
      rodape.append(
        entrada,
        el('div', { className: 'midia-galeria__edicao-acoes' }, [
          el('button', {
    ...
    if (podeEditar) {
      item.appendChild(el('div', { className: 'midia-galeria__acoes' }, [
        el('button', {
          className: 'btn btn--text btn--sm',
          type: 'button',
        ...
        el('button', {
          className: 'btn btn--text btn--sm midia-galeria__remover',
          type: 'button',
          onClick: async (e) => {
            e.stopPropagation();
            ...
            if (!ok) return;
            try {
              await servico.excluir(imagem.id);
  ...
  async function recarregar() {
    ...
    grade.appendChild(el('p', {
      className: 'midia-galeria__aviso', textContent: 'Carregando...',
    }));
    try {
      itens = await servico.listar(id);
    } catch (err) {
      if (disposed) return;
      clearChildren(grade);
      grade.appendChild(el('p', {
    ...
    if (!itens.length) {
      grade.appendChild(el('p', {
        className: 'midia-galeria__vazio',
        textContent: podeEditar ? vazioEditavel : vazio,
      }));
      return;
    }
