...
const GERAL = [
  ...
  'DGEO atende. Leitura exige login; toda ESCRITA exige administrador. Publicos, sem',
  'login: os GET de /api/mapoteca/dominio e a consulta por localizador.',
...
const REGRAS = {
  ...
  item: [
    'Todo item aponta EXATAMENTE UM produto identificado: uuid_versao (acervo) OU',
    'produto_avulso_id. Mandar os dois, ou nenhum, e recusado.',
    'O CORTE E DE POSSE, nao de formato: o acervo guarda o que E NOSSO, com versao e',
    'arquivo; o avulso guarda o que SO PASSOU PELA IMPRESSORA (papel quadriculado,',
    'carta de outro CGEO, impresso de ocasiao).',
    'Folha nossa que ainda nao esta catalogada NAO vira avulso: ela vira item quando',
    'entrar no acervo. Usar avulso ali esconde a lacuna de catalogacao. Veja',
    '"mapoteca produto_avulso listar" e a rota de reconciliacao antes de criar um.',
    ...
    'pedido explicito.',
    'forma_entrega_id e data_entrega ficam NULOS no cadastro: sao do ato da entrega,',
    'nao do pedido.',
  ...
  produto_avulso: [
    'E o que a mapoteca imprime SEM ser produto do acervo. Dois casos reais de 2026:',
    'papel quadriculado (100 folhas de 80 x 68 cm, quadricula 4 x 4 cm, para Central de',
    'Tiro) e carta de OUTRO CGEO, que a mapoteca distribui e nao cataloga.',
    'O corte e de POSSE, nao de formato. Se o produto e NOSSO, o lugar dele e o acervo,',
    'mesmo que ainda nao esteja catalogado: nesse caso o item espera a catalogacao.',
    'So o nome e obrigatorio. mi, tipo_produto e escala sao opcionais porque a carta de',
    'outra area tem MI e o papel quadriculado nao tem MI, nem escala, nem cabe em',
    'nenhum dos quatro tipos do dominio. Forcar um deles mente no relatorio.',
    'A dimensao fisica vai na descricao ("80 x 68 cm, quadricula de 4 x 4 cm"). Essa',
    'descricao E PUBLICA: sai na consulta do cliente por localizador. Anotacao interna',
    'nao vai ai.',
    'NAO ha trava de banco impedindo MI aqui, porque a carta de outro CGEO tem MI',
    'legitimo. Quem cuida disso e a reconciliacao: GET /produto_avulso/reconciliacao',
    'lista avulso cujo MI JA existe no acervo, e a coluna vezes_pedido mostra o que se',
    'imprime com frequencia e provavelmente deveria estar catalogado.',
    'Avulso ja usado em item de pedido NAO pode ser apagado (o servidor recusa com 400):',
    'apagar apagaria o registro do que foi impresso. Para tirar de circulacao use',
    'ativo = false.'
  ],
