...
// auth - CLI de IDENTIDADE do SCA, desenhado para AGENTES.
//
...
// MODULO, e este fala com a plataforma: usuario, perfil por modulo, senha e
// historico de acesso nao pertencem a acervo, mapoteca nem orcamento.
//
// Existe desde 2026-08-02, quando a autenticacao veio para dentro do SCA e o
// Auth Server externo saiu de cena.
//
...
const AJUDA = `auth - CLI de identidade do SCA (usuarios, perfis, senhas, acessos), para agentes

CONTRATO (nao gasta rede, leia isto antes de montar um corpo)
  auth schema                      lista os recursos e suas operacoes
  auth schema usuario              campos, tipos, obrigatorios e regras
  auth usuario dominios            posto/graduacao, modulos e niveis de perfil

USUARIOS  (exigem ADMINISTRADOR)
  auth usuario listar [--ativo false] [--admin] [--sem-senha] [--modulo acervo]
  auth usuario obter --uuid U
  auth usuario criar --data '{...}' [--dry-run]
  auth usuario editar --uuid U --data '{"administrador": false, "ativo": true}'
  auth usuario editar-lista --data '{"usuarios": [...]}' --confirmar <quantos>
  auth usuario excluir --uuid U --confirmar U
  auth usuario resetar-senha --uuids U1,U2 --confirmar <quantos>
  auth usuario perfis --uuid U
  auth usuario perfis --uuid U --conceder acervo=operador --revogar mapoteca --confirmar U
...
ACESSOS  (exigem ADMINISTRADOR)
  ...
  auth acessos logins dia|mes|usuarios|clientes [--total N] [--max N]

SESSAO
  auth status                      o SCA esta no ar? ha token em cache? sou admin?
  auth login                       autentica uma vez e guarda o token (~1h)
  auth logout                      descarta o token em cache
...
/api/usuarios e /api/acessos sao rotas de PLATAFORMA: sem prefixo de modulo e
guardadas por verifyAdmin. Nao existe "administrador de modulo", e o
administrador e global. NAO ha comando de aplicacao: o catalogo dgeo.aplicacao
do Auth Server nao veio na fusao de 2026-08-02.`
...
async function principal () {
  ...
  if (!modulo) {
    process.stderr.write(
      ...
      'Use auth --ajuda para o mapa completo.\n'
    )
    return 1
  }
