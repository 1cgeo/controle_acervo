...
test('todo recurso da registry renderiza contrato sem quebrar', () => {
  for (const chave of Object.keys(RECURSOS)) {
    ...
    for (const acao of Object.keys(RECURSOS[chave].operacoes)) {
      assert.ok(texto.includes(`auth ${chave} ${acao}`), `${chave} nao listou ${acao}`)
    }
  }
})
...
// O catalogo `dgeo.aplicacao` do Auth Server nao veio na fusao de 2026-08-02,
// por decisao do chefe. Um recurso `aplicacao` aqui seria contrato inventado.
