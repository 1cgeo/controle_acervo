...
async function autenticar (cfg) {
  ...
  try {
    ...
    if (err instanceof ErroHttp) {
      // A mensagem do servidor vem PRIMEIRO e inteira: desde 2026-08-02 quem
      // valida a senha e o proprio SCA, e ele distingue tres casos que a frase
      // generica juntaria num so ("usuario ou senha invalida"): conta inativa,
      // senha errada e senha AINDA NAO CADASTRADA (o hash nao foi copiado do
      // Auth Server). O ultimo nao se resolve tentando de novo.
      throw new Error(
        `Falha no login (HTTP ${err.status}): ${err.mensagem}. ` +
        'Confira o usuario e a senha; se a resposta fala em senha nao cadastrada, ' +
        'o caminho e um administrador rodar: auth usuario resetar-senha.'
      )
    }
    throw err
  }
