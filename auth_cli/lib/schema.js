...
function blocoCampos (schemaJoi, titulo, avisarStrip) {
  ...
  if (avisarStrip) {
    ...
    linhas.push('    o auth avisa quando isso acontece.')
  }
  return linhas
}
...
/** Indice curto de todos os recursos, para o `auth schema` sem argumento. */
...
function explicarErro (schemaJoi, erros, dica, titulo) {
  ...
  linhas.push('')
  linhas.push(dica || 'contrato completo: auth schema <recurso>')
  return linhas.join('\n')
}
