...
// verdade.
//
// Diferenca deliberada em relacao ao CLI de origem, no servico de autenticacao:
// la a listagem de usuarios era PUBLICA e resolver os nomes nao gastava token.
// Aqui `GET /usuarios` e `verifyAdmin`, entao a resolucao passa pela sessao.
// Continua valendo a pena: uma requisicao a mais e barata perto de confirmar as
// cegas, e quem chama estas rotas ja e administrador de qualquer forma.
...
function validar (schemaJoi, corpo, acao) {
  ...
  if (r.descartados.length) {
    ...
    avisos.push(
      ...
      '        Confira o nome em: auth schema usuario'
    )
  }

  if (!r.ok) {
    const erro = new Error(esquema.explicarErro(
      schemaJoi, r.erros, `contrato completo: auth schema usuario   (operacao ${acao})`
    ))
...
async function executar (args, cfg, injetadas) {
  ...
  switch (acao) {
    ...
    case 'obter': {
      ...
      if (!achado) {
        throw recusa([
          ...
          '',
          'Confira a lista: auth usuario listar',
    ...
    case 'criar': {
      ...
      if (!bruto || typeof bruto !== 'object') {
        throw new Error(
          ...
          'Contrato: auth schema usuario\n' +
          'Resolva tipo_posto_grad_id (e os nomes de modulo) antes com: auth usuario dominios'
        )
      }
      ...
      if (!corpo.perfis || !Object.keys(corpo.perfis).length) {
        avisos.push(
          ...
          'auth usuario perfis --uuid <uuid> --conceder acervo=consulta'
        )
      }
    ...
    case 'editar': {
      ...
      if (!bruto || typeof bruto !== 'object') {
        throw new Error(
          "editar exige --data '{...}'. Contrato: auth schema usuario\n" +
    ...
    case 'editar-lista': {
      ...
      if (!bruto || typeof bruto !== 'object') {
        throw new Error(
          "editar-lista exige --data '{\"usuarios\": [...]}'. Contrato: auth schema usuario"
        )
      }
      ...
      if (flags.confirmar !== String(uuids.length)) {
        linhas.push(
          ...
          `  auth usuario editar-lista --data '...' --confirmar ${uuids.length}`,
    ...
    case 'excluir': {
      ...
      linhas.push(
        ...
        `  auth usuario editar --uuid ${uuid} --data '{"administrador": false, "ativo": false}'`
      )
      ...
      if (flags.confirmar !== String(uuid)) {
        linhas.push(
          ...
          `  auth usuario excluir --uuid ${uuid} --confirmar ${uuid}`
        )
        throw recusa(linhas)
      }
    ...
    case 'resetar-senha': {
      ...
      if (!uuids || !uuids.length) {
        throw new Error(
          ...
          'Descubra com: auth usuario listar'
        )
      }
      ...
      linhas.push(
        ...
        '(auth usuario trocar-senha), senao o acesso fica aberto.',
      ...
      if (flags.confirmar !== String(uuids.length)) {
        linhas.push(
          '',
          'Nao confirmado. Para executar, informe a QUANTIDADE em --confirmar:',
          `  auth usuario resetar-senha --uuids ${uuids.join(',')} --confirmar ${uuids.length}`,
          'Para so ver a requisicao que sairia: acrescente --dry-run.'
        )
        throw recusa(linhas, avisos)
      }
      ...
      if (!concedidos.length && !revogados.length) {
        ...
        return {
          texto: [
            ...
            'Para mudar:',
            `  auth usuario perfis --uuid ${uuid} --conceder acervo=consulta`,
            `  auth usuario perfis --uuid ${uuid} --revogar mapoteca --confirmar ${uuid}`
          ].join('\n')
        }
      }
      ...
      if (perigosas.length) {
        ...
        if (flags.confirmar !== String(uuid)) {
          linhas.push(
            ...
            `  auth usuario perfis --uuid ${uuid} ${concedidos.map(c => `--conceder ${c}`).concat(revogados.map(r => `--revogar ${r}`)).join(' ')} --confirmar ${uuid}`
          )
          throw recusa(linhas)
        }
      }
