...
// `auth acessos ...` - o historico de acesso (dgeo.login), so leitura.
//
//   auth acessos resumo
//   auth acessos logados
//   auth acessos logins dia       [--total 14]
//   auth acessos logins mes       [--total 12]
//   auth acessos logins usuarios  [--total 30] [--max 10]
//   auth acessos logins clientes  [--total 30]
//
...
// Os quatro recortes de `logins`, na ordem em que a tela os mostra.
const SERIES = {
  dia: 'logins-dia',
  mes: 'logins-mes',
  usuarios: 'logins-usuarios',
  clientes: 'logins-clientes'
}
...
function resolverOperacao (args) {
  ...
  if (acao === 'logins') {
    ...
    if (!serie) {
      throw new Error(
        'auth acessos logins exige o recorte: ' + Object.keys(SERIES).join(', ') + '.\n' +
        '  ex.: auth acessos logins dia --total 30'
      )
    }
  ...
  if (!nome) {
    return {
      texto: [
        ...
        'contrato completo: auth schema acessos'
      ].join('\n')
    }
  }
  ...
  if (operacao.query) {
    ...
    if (!r.ok) {
      const erro = new Error(esquema.explicarErro(
        ...
        `filtros aceitos: ${aceitos.join(', ')}   (contrato: auth schema acessos)`,
...
/**
 * `auth acessos` sozinho lista as operacoes, e errar o nome do recorte tambem se
