...
// logout, senao "encerrei a sessao do auth" parece um efeito menor do que e.
...
async function executar (args, cfg) {
    ...
    if (token) {
      ...
      try {
        ...
        if (!dados.administrador) {
          ...
          linhas.push('respondem: auth usuario meu-perfil e auth usuario trocar-senha.')
        }
      } catch (err) {
  ...
  const avisos = administrador
    ...
    : ['Autenticado, porem NAO administrador: /api/usuarios e /api/acessos vao voltar ' +
       ...
       '"administrador de modulo"). Continuam funcionando: auth usuario meu-perfil e ' +
       'auth usuario trocar-senha.']
