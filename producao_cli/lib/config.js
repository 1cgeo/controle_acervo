...
// O CLI nao e o QGIS: entre os clientes aceitos, 'sca_web' e o honesto para uma
// ferramenta que fala com a API pela rede, e e o que os CLIs irmaos (acervo,
// mapoteca, orcamento, auth) ja usam. O valor entra em `dgeo.login.cliente`.
...
function resolver (flags, exigirServidor = true) {
  ...
  if (!server && exigirServidor) {
    throw new Error(
      'Informe --server ou a variável de ambiente SCA_URL (ex.: http://IP:porta).'
    )
  }
  ...
  if (aceitos.length && !aceitos.includes(cliente)) {
    throw new Error(
      `Cliente de auth "${cliente}" não é aceito pelo SCA. Aceitos: ${aceitos.join(', ')}.`
    )
  }
