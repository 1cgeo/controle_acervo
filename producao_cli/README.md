...
| escrever qualquer coisa do PIT | `verifyAdmin` |
| o `/api/rpcmtec` inteiro, **inclusive a leitura** | `verifyAdmin` |

`verifyGerente` lê o banco a cada requisição, e não o token: rebaixar perfil vale na hora. O `producao login` diz, numa linha, o que a pessoa alcança, para o 403 não ser lido como rota quebrada.
