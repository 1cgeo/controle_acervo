...
const GERAL = [
  ...
  '',
  'Acesso: publico (sem login) sao so /api (health) e /api/integracao/*. Todo o',
  'resto exige perfil no modulo acervo, inclusive os GET de dominio em',
  '/api/gerencia/dominio/*: consulta le, operador cataloga, gerente exclui. O',
  'administrador e global e passa em tudo.',
  '',
  'A EXCECAO e o recurso rpcmtec, que nao e do modulo acervo: LER pede gerente de',
  'QUALQUER modulo (verifyGerente) e ASSINAR pede administrador. Perfil no acervo',
  'nao abre o relatorio, e perfil na mapoteca abre.',
  '',
...
const REGRAS = {
  ...
  arquivo: [
    ...
    'A resposta do confirm-upload traz os ids REAIS do que foi criado:',
    'versoes[].versao_id e versoes[].produto_id em preparar-versao, e',
    'produtos[].produto_id com produtos[].versoes[].versao_id em preparar-produto.',
    'Use esses ids direto. Ate 2026-08-05 a resposta trazia o id da tabela',
    'temporaria da sessao, que nao aponta para versao nenhuma do acervo.',
  ...
  rpcmtec: [
    ...
    'demanda: abre-se a edicao do mes uma vez, e o documento se le pelo id dela.',
    'Edicao ABERTA calcula do banco a cada leitura; edicao FECHADA devolve o',
    'congelado, que e o que foi assinado. A conferencia poe os dois lado a lado.',
    'Reabrir descongela, e o texto congelado deixa de ser o que a rota devolve.',
    'O RPCMTec cobre a Divisao inteira na numeracao do documento: sao 33 blocos,',
    '21 calculados do banco, 11 digitados e 1 fixo, repartidos entre acervo,',
    'mapoteca, orcamento, pit, efetivo e equipamento. Eram 34 ate 2026-08-08,',
    'quando a 7.3 se fundiu na 7.2. --anuario baixa o Anuario Estatistico (.ods)',
    'do mesmo mes, que sobe para a DSG no mesmo envio e sai da planilha-semente da',
    'propria DSG; ele nao passa pela edicao mensal.',
    'A GUARDA MUDOU EM 2026-08-08, e ate ali era admin em tudo. LER (listar, ler',
    'o documento, conferir, PDF, anexos, Anuario) e verifyGerente: administrador',
    'global OU gerente de QUALQUER modulo, porque o relatorio e a prestacao de',
    'contas da Divisao e sao os gerentes que a conferem antes da assinatura.',
    'ASSINAR (abrir o mes, editar assinante e data, fechar, reabrir, anexar e',
    'apagar o assinado) continua do ADMINISTRADOR.',
    'ESCREVER UMA SUBSECAO e recortado pelo modulo DELA (verifyGerente mais',
    'verifyModuloSubsecao): o gerente da mapoteca le a secao 4 do orcamento e nao',
    'altera uma linha dela. Essas rotas nao estao nesta registry por isso.'
  ],
