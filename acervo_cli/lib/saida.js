...
// janela de contexto finita: o /produto/detalhado de uma carta com quatro
// edicoes devolve produto + versoes + relacionamentos + arquivos aninhados, e o
// JSON cru disso e cerca de dez vezes o mesmo conteudo em TSV recortado.
//
...
const EH_TAMANHO = /^(tamanho_mb|capacidade_gb|gb_)/

function celula (chave, valor) {
  if (valor === null || valor === undefined) return '-'
  if (valor === '') return '-'
  if (typeof valor === 'boolean') return valor ? 'sim' : 'nao'
  if (EH_TAMANHO.test(chave)) return numero(valor)
  // Array curto de escalares (edicoes_topo, palavras_chave) cabe inteiro; array
  // de objetos vira so a contagem, senao uma linha de TSV explode a janela.
  ...
  if (typeof valor === 'object') {
    ...
    return chaves.length ? `{${chaves.length}}` : '-'
  }
  const texto = String(valor)
  // Datas ISO com hora viram so a data: no acervo o que importa e o DIA da
  // edicao (a hora e sempre 00:00 vinda do banco) e ela custa 14 caracteres.
