...
// (ler a versao e devolver o PUT logo em seguida, por exemplo).
 ...
 * a mensagem que o backend produz (as traducoes de violacao de FK, de CHECK e
 * dos triggers de acervo chegam aqui prontas e sao boas: so propagar).
 *
 * O SCA aceita corpo em DELETE (varias rotas de exclusao levam a lista de ids
 * no body). Por isso `corpo` nao e restrito a POST/PUT.
 */
...
async function autenticar (cfg) {
  ...
  return { token: dados.token, administrador: dados.administrador === true }
}
