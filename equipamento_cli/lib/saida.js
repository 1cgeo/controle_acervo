...
// janela de contexto finita: a ficha de um bem com quatro historicos em JSON
// indentado e varias vezes o mesmo conteudo em TSV recortado.
//
...
// Colunas tratadas como dinheiro na saida legivel. No equipamento o dinheiro
// esta todo na manutencao (valor, valor_orcado, valor_pdr) e no cartao de custo
// do painel.
//
// `vida_util_meses` NAO cai aqui, e nao pode cair: ela e uma contagem de MESES,
// e formata-la como moeda ("120,00") sugeriria casa decimal onde nao ha.
...
function celula (chave, valor) {
  ...
  if (Array.isArray(valor)) return valor.length ? String(valor.length) + ' item(ns)' : '-'
  if (typeof valor === 'object') return JSON.stringify(valor)
  const texto = String(valor)
  // Datas ISO com hora viram so a data: as onze colunas DATE do modulo sao dia
  // de calendario, e a hora custa caracteres sem responder nenhuma pergunta.
