...
| `/api/campo` | **produção** | A atividade que a Divisão executa FORA dela: reambulação, voo de drone, ponto de controle, modelo 3D e panorâmica 360. É a fonte da subseção 2.5 do RPCMTec, que era DIGITADA da tela do SAP até 2026-08-08. Prefixo próprio, mas **não é módulo**: `dominio.modulo` continua com seis linhas, e a guarda cobra `producao` -- campo é o trabalho que o PIT promete. Ler a lista, o mapa, as fotos e os trajetos é de **consulta**; cadastrar, corrigir, subir foto e importar trajeto é de **operador**; **apagar o campo é de gerente**, porque o CASCADE leva as fotos e os trajetos junto. `/imagem/:id/arquivo` responde binário, e é a única rota que não sai por `sendJsonAndLog` |
