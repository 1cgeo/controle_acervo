...
| `/api/rpcmtec` | plataforma | A edição mensal do RPCMTec, o documento e o PDF assinado, o Anuário Estatístico e o RTM/META4 (ODS), mais a capacitação ministrada (módulo `pit`, subseção 2.6) e a recebida (módulo `efetivo`, 6.2). As duas aceitam FOTO e VÍDEO, e `/capacitacao/<tipo>/imagem/:imagemId/arquivo` responde binário |
...
| `rpcmtec` | `edicao`, `subsecao` e `subsecao_revisao`, `anexo_edicao`, mais `capacitacao`, `capacitacao_militar` e `capacitacao_imagem` (foto e vídeo em `bytea`, como `campo.imagem`) -- a ENTRADA digitada das subseções 2.6 e 6.2. São **33 subseções** no gerador (`rpcmtec_estrutura.js`), e as CALCULADAS saem de consulta, nunca de linha gravada |
